...
#        PROMPT=$(printf "%q" "$DECODED_CONTENT")
       ...
       escaped_payload=$(echo "$payload" | jq -c .)
        ...
        curl https://api.openai.com/v1/completions -H "Content-Type: application/json" -H "Authorization: Bearer $API_KEY" -d "{\"prompt\":\"$escaped_payload\",\"model\":\"babbage\"}" > suggestions.txt

        echo "STARTING A PUTARIA!!!!"
        # Add suggestions as a comment on the pull request
        curl \
          -X POST \
          -H "Accept: application/vnd.github3+json" \
          -H "Authorization: token $GITHUB_TOKEN"\
          -H "Content-Type: application/json" \
          -d "{\"body\":\"$(cat suggestions.txt | jq -r '.text')"} \
